...
export default async function HomePage() {
  ...
  return <SiteShell>
    ...
    {visible.featured_products && <section className="mt-14"><div className="flex justify-between gap-3"><div><p className="text-sm font-bold uppercase tracking-widest text-[#2c9873]">Selected tools</p><h2 className="mt-2 text-3xl font-bold">Featured products</h2></div><Link href="/products" className="font-semibold text-[#165e46]">View all →</Link></div><div className="mt-6 grid gap-6 md:grid-cols-2 lg:grid-cols-4">{data.hero_products.map(product => <ProductCard key={product.id} product={product} placement="homepage_featured" />)}</div>{data.hero_products.length === 0 && <p className="mt-6 text-[#567069]">Products will appear here when published.</p>}</section>}
    {visible.featured_brands && data.featured_brands.length > 0 && <section className="mt-14"><h2 className="text-3xl font-bold">Featured brands</h2><div className="mt-6 grid gap-3 sm:grid-cols-2 md:grid-cols-4">{data.featured_brands.map(brand => <Link key={brand.id} href={`/brands/${brand.slug}`} className="rounded-xl border bg-white p-5 text-center font-bold hover:border-[#2c9873]">{brand.name}</Link>)}</div></section>}

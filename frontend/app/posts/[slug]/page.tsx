...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
 ...
 try { const result = await publicGet<Detail>(`posts/${encodeURIComponent(slug)}`); const post = result.data.post; return { title: post.seo?.meta_title || post.title, description: post.seo?.meta_description || post.excerpt, alternates: { canonical: post.seo?.canonical_url }, openGraph: { images: post.seo?.open_graph?.image ? [post.seo.open_graph.image] : [] } }; } catch { return { title: "Article" }; }
}

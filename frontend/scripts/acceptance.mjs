...
(async()=>{
 ...
 await check('Mobile public homepage',async()=>{await page.goto(base+'/');await page.locator('h1').waitFor();const latest=page.getByRole('region',{name:'Latest Affiliate Products'}),cards=latest.getByRole('article');const first=await cards.first().boundingBox(),second=await cards.nth(1).boundingBox();if(!first||!second||first.width<300||Math.abs(second.x-first.x)>5||second.y<=first.y)throw Error('Mobile latest grid should show one card per row');const menu=page.getByRole('button',{name:'Open navigation'});await menu.click();if(await page.getByRole('button',{name:'Close navigation'}).getAttribute('aria-expanded')!=='true'||!await page.getByRole('navigation',{name:'Mobile navigation'}).isVisible())throw Error('Mobile menu failed to open');await page.getByRole('navigation',{name:'Mobile navigation'}).getByRole('link',{name:'Products'}).click();await page.waitForURL('**/products');await page.goto(base+'/');if(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth))throw Error('Horizontal overflow');await page.screenshot({path:path.join(output,'mobile-home.png'),fullPage:true});});

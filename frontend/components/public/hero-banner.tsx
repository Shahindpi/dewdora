...
export function HeroBannerSection({ banner }: { banner?: HeroBanner }) {
  ...
  return <section className="relative mt-10 min-h-80 overflow-hidden rounded-3xl bg-[#165e46] text-white">
    ...
    <div className="relative flex min-h-80 max-w-2xl flex-col justify-center px-8 py-14 md:px-16"><h2 className="text-4xl font-black md:text-5xl">{banner.heading}</h2>{banner.description && <p className="mt-5 text-lg text-white/90">{banner.description}</p>}{banner.cta_text && banner.cta_url && (banner.cta_url.startsWith("/") ? <Link href={banner.cta_url} className="mt-7 w-fit rounded-lg bg-white px-6 py-3 font-bold text-[#165e46]">{banner.cta_text}</Link> : <a href={banner.cta_url} className="mt-7 w-fit rounded-lg bg-white px-6 py-3 font-bold text-[#165e46]">{banner.cta_text}</a>)}</div>
  </section>;
}

...
export function PostCard({ post }: { post: PublicPost }) {
  return <Link href={routes.posts.show(post.slug)} className="group overflow-hidden rounded-2xl border border-[#dce6d9] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
    {post.featured_image && <Image width={720} height={384} sizes="(max-width: 768px) 100vw, 33vw" src={imageUrl(post.featured_image)} alt={`${post.title} featured image`} className="h-48 w-full object-cover" />}
